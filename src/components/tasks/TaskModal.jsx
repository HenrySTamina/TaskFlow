  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <section
        ...
        className="taskflow-modal-enter relative max-h-[calc(100vh-2rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-2xl"
      >
        <header className="flex items-start justify-between border-b border-slate-100 p-5 sm:p-6">
          ...
          <button
            ...
            aria-label="Cerrar"
            onClick={onClose}
            className="taskflow-interactive rounded-xl p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
          >
            <X className="taskflow-icon" size={21} />
          </button>
        </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-5 sm:p-6">
          <label className="block">
            ...
            {titleError && (
              <span className="taskflow-status-enter mt-2 block text-sm font-medium text-red-600">
                {titleError}
              </span>
            )}
          </label>
          ...
          <footer className="flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
            ...
            <button
              type="submit"
              className="taskflow-interactive rounded-xl bg-indigo-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-200 transition hover:bg-indigo-700"
            >
